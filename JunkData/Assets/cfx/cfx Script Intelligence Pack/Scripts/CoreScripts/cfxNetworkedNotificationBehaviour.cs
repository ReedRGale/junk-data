 ...
 *
 * THIS IS THE NETWORKED VERSION. 
 *
 * Any object that you attach a child of this script must also have a 
 * a Network Identity or you will receive warnings and errors.
 * 
 * The networked version of SIP works with the non-networked SIP
 * cfxNotificationNanager for all local broadcasts, plus adds the 
 * ability to send notifications to all other clients on the
 * network using the networkSendMessage methods  
 * 
 */
public class cfxNetworkedNotificationBehaviour : NetworkBehaviour {
	...
	public float fetchFloat(Dictionary<string, object> info, string theKey, float defaultValue) {
        ...
        if (theFloatString == null) {
            Debug.Log("fetchString: no such key: " + theKey + ", returning default: " + defaultValue);
            return defaultValue;
        }
		...
		if (float.TryParse (theFloatString, out theFloat)) {
            // note: we might as well just drop through, and return theFloat at the end;
            // this way, however, we could add some more debug code
            //Debug.Log("fetchFloat returns " + theFloat);
			return theFloat;
		};
		if (verbose)
...
//		bool theBool = defaultValue;
    ...
    // this is usually required when you have a persistent object and need to re-connect on scene load
